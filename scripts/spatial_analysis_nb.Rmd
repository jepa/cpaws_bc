...
agg_all_scen <- function(scenario){
  ...
  dbem_paths <- taxon_list %>% 
    ...
    select(taxon_key) %>% 
    mutate(
      dbem_no_mpa_path = paste0("~/Library/CloudStorage/OneDrive-UBC/Data/dbem/dbem_cmip6/r_data/C6GFDL85F1_t2/",taxon_key,variable,".RData"),
      dbem_mpa_path = paste0("/Volumes/Enterprise/Data/cpaws_bc/Data/dbem_runs/rdata/c6gfdl85f1cpaws",scenario,"/",taxon_key,variable,".RData")
    )
...
write_csv(agg_per_species_abd_scen, my_path("R","tables","agg_per_species_abd_gfdl85.csv"))
...
# read data
agg_per_species_abd <- my_path("R","tables","agg_per_species_abd_gfdl85.csv",read = T) %>% 
...
# Raw data
plot_agg_per_species_abd <- agg_per_species_abd_scen %>% 
  left_join(taxon_list) %>% 
  filter(run == "mpa") %>% 
  ggplot() +
  geom_line(
...
ggsave(my_path("R","figures","agg_per_species_abd_gfdl85.png"),
...
diff_plot <-
  ...
  spread(scenario,sum) %>% 
  mutate(
    d_25 = my_chng(`100`,`25`,limit = 100),
    d_50 = my_chng(`100`,`50`,limit = 100),
    d_75 = my_chng(`100`,`75`,limit = 100)
  ) %>% 
...
ggsave(my_path("R","figures","scen_abs_diff_per_species_abd_gfdl85.png"),
       plot = diff_plot,
       width = 20,
       height = 15)
...
ggsave(my_path("R","figures","time_diff_per_species_abd_gfdl85_scen.png"),
       plot = last_plot(),
       width = 10,
       height = 8)
...
ggsave(my_path("R","figures","delta_delta_per_species_abd_gfdl85_scen.png"),
       plot = last_plot(),
       width = 10,
       height = 8)
...
ggsave(my_path("R","figures/totals","agg_total_abd_gfdl85.png"),
       plot = last_plot(),
       width = 10,
       height = 7)
```
...
ggsave(my_path("R","figures/totals/","scen_abs_diff_total_abd_gfdl85.png"),
       plot = last_plot(),
       width = 10,
       height = 7)
...
ggsave(my_path("R","figures/totals","time_diff_agg_abd_gfdl85_scen.png"),
       plot = last_plot(),
       width = 10,
       height = 8)
...
ggsave(my_path("R","figures/totals","delta_delta_total_abd_gfdl85_scen.png"),
       plot = last_plot(),
       width = 8,
       height = 4)
