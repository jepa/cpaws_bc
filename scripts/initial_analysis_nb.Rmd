...
scenarios_df <- mpa_grid %>% 
  mutate(
    scenario_25 = ifelse(status == "protected",f_mort*0.25,f_mort),
    scenario_50 = ifelse(status == "protected",f_mort*0.5,f_mort),
    scenario_75 = ifelse(status == "protected",f_mort*0.75,f_mort),
    scenario_100 = ifelse(status == "protected",f_mort*0,f_mort)
  )
...
scenarios_map <- scenarios_df %>% 
  gather("scen","value",scenario_25:scenario_100) %>% 
   left_join(dbem_grid) %>% 
  filter(lat > 49,
         lat < 60,
         lon < -120,
  ...
  # geom_sf(data = mpan_sf, aes(), fill = NA, color = "white") +
  facet_wrap(~scen) +
  scale_fill_viridis_d("Fishing mortality (fraction)") +
  scale_color_viridis_d("Fishing mortality (fraction)")
