...
# Function to estimate the total catch at the whole EEZ for both MPA and no-MPA runs
agg_per_spp_mcp <- function(dbem_paths, grid = F){
  
  for(i in 1:nrow(dbem_paths)){
    # print(dbem_paths$dbem_mpa_path[i])
    # print(i)
    # Load MPA run
    ...
    no_mpa_data <- as.data.frame(sppMCPfnl) %>% 
      rowid_to_column("index") %>% 
      filter(index %in% bc_grid) %>% 
      mutate(run = "no_mpa",
             variable = variable)
